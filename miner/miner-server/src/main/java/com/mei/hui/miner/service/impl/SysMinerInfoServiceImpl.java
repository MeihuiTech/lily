...
public class SysMinerInfoServiceImpl extends ServiceImpl<SysMinerInfoMapper,SysMinerInfo> implements ISysMinerInfoService
{
    ...
    public SysMinerInfo selectSysMinerInfoById(Long id)
    {
        ...
        if(powerAvailable != null && powerAvailable.compareTo(BigDecimal.ZERO) > 0){
            miner.setLuckyValue((new BigDecimal(twentyFourTotalBlocks)).multiply(new BigDecimal(100)).divide(powerAvailable.divide(power,5, BigDecimal.ROUND_UP).multiply(new BigDecimal(14400)),2, BigDecimal.ROUND_UP));
        } else {
    ...
    public Map<String,Object> minerPagelist(SysMinerInfoBO sysMinerInfoBO){
        ...
        for (SysMinerInfo minerInfo:page.getRecords()) {
            ...
            if(power != null && power.compareTo(BigDecimal.ZERO) ==1 && allPowerAvailable != null && allPowerAvailable.compareTo(BigDecimal.ZERO) ==1){
                sysMinerInfoVO.setLuckyValue((new BigDecimal(twentyFourTotalBlocks)).multiply(new BigDecimal(100)).divide(allPowerAvailable.divide(power,5, BigDecimal.ROUND_UP).multiply(new BigDecimal(14400)),2, BigDecimal.ROUND_UP));
            }
